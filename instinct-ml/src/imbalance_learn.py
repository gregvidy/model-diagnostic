    ...
    def _initialize_sampler(self):
        ...
        if self.sampler_type == 'upsample':
            if self.algorithm == 'RandomOverSampler':
                self._sampler = RandomOverSampler(sampling_strategy=self.target_ratio,
                                                  random_state=self.random_state)
            elif self.algorithm == 'SMOTE':
                self._sampler = SMOTE(sampling_strategy=self.target_ratio,
                                      random_state=self.random_state)
            elif self.algorithm == 'ADASYN':
                self._sampler = ADASYN(sampling_strategy=self.target_ratio,
                                       random_state=self.random_state)
            else:
                raise ValueError(
            ...
            if self.algorithm == 'RandomUnderSampler':
                self._sampler = RandomUnderSampler(sampling_strategy=self.target_ratio,
                                                   random_state=self.random_state)
                ...
                self._sampler = NearMiss(version=version, sampling_strategy=self.target_ratio)
            elif self.algorithm == 'CondensedNearestNeighbour':
                self._sampler = CondensedNearestNeighbour(sampling_strategy=self.target_ratio,
                                                         random_state=self.random_state)
            elif self.algorithm == 'EditedNearestNeighbours':
                self._sampler = EditedNearestNeighbours(sampling_strategy=self.target_ratio)
            elif self.algorithm == 'TomekLinks':
                self._sampler = TomekLinks(sampling_strategy=self.target_ratio)
            else:
                raise ValueError(
...
if __name__ == "__main__":
    ...
    # Split the dataset - using only training set for resampling examples
    ...
    # Example 1: Upsample minority class using SMOTE to target ratio 0.5
    ...
    # Example 2: Upsample using RandomOverSampler to achieve balanced classes (1:1 ratio)
    ...
    # Example 3: Upsample using ADASYN to target ratio 0.75
    ...
    # Example 4: Downsample using RandomUnderSampler to achieve balanced classes
    ...
    # Example 5: Downsample using NearMiss-1 to target ratio 0.8
    ...
    # Example 6: Downsample using TomekLinks to remove overlapping instances
    print("\nAttempting TomekLinks downsampling...")
