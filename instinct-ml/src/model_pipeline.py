...
def preprocess_numerics_and_bools_core(
    ...
    for col in df_processed.select_dtypes(include=["string","object"]).columns:
        ...
        if col.startswith("Is") and isinstance(col, str) and df_processed[col].dtype in [
            "object",
            "int64",
            "bool",
        ]:
...
class ModelPipeline:
    ...
    def prepare_data(
        ...
        encoding_type: str = "OneHot", # Options: "OneHot", "Frequency", "Direct"
        ...
        if not current_object_cols:
            ...
            X_transformed = X_processed_core
        else:
            # Always fit/transform categorical columns to codes first
            if is_training:
                X_transformed = self.category_manager.fit_transform(
                    X_processed_core, categorical_columns=current_object_cols
                )
            else:
            ...
            if encoding_type == "Frequency":
                ...
                # Convert coded categorical columns to category dtype for one-hot encoding
                cat_cols_to_encode = [col for col in current_object_cols if col in X_transformed.columns]
                print(f"Applying one-hot encoding to columns: {cat_cols_to_encode}")
                for col in cat_cols_to_encode:
                    X_transformed[col] = X_transformed[col].astype('category')
                X_transformed = pd.get_dummies(X_transformed, columns=cat_cols_to_encode, drop_first=True)
            elif encoding_type == "Direct":
                # Keep categorical columns as numerical codes (already done by CategoryManager)
                print("Using direct label encoding (codes).")
            else:
    ...
    def build_pipeline(self):
        ...
        Since preprocessing is handled by prepare_data(), we just use the model directly.
        """
        print("Preparing model for training...")
        # Use the model directly since all preprocessing is done in prepare_data()
    ...
    def train(
        ...
        if self.pipeline is None:
            raise RuntimeError("Model not built. Call build_pipeline() first.")
        ...
        if show_training_log and self.model_type == "xgboost":
            self.pipeline.fit(X_train, y_train, eval_set=[(X_train, y_train)], verbose=True)
        elif show_training_log and self.model_type == "lightgbm":
            self.pipeline.fit(X_train, y_train, callbacks=[lgb.log_evaluation(period=50)])
        else:
    ...
    def cross_validate(
        ...
        for i, (tr_idx, te_idx) in enumerate(
            ...
            # Build a fresh model for each fold to ensure independent training
            fold_model = self._get_model(self.model_type)

            # Train the fold's model
            fold_model.fit(X_tr_final, y_tr)

            # Evaluate on the test set of the current fold
            y_prob = fold_model.predict_proba(X_te_final)[:, 1]
    ...
    def get_feature_importance(self, X: pd.DataFrame, y: pd.Series):
        ...
        if hasattr(self.pipeline, "feature_importances_"):
            ...
            return pd.DataFrame(
                {"feature": X.columns, "importance": self.pipeline.feature_importances_}
    ...
    def get_pipeline(self):
        """
        Returns the trained model (for backward compatibility).
        """
        if self.pipeline is None:
