...
class ImbalancedSampler:
    ...
    def _initialize_sampler(self):
        ...
        if self.sampler_type == "upsample":
            ...
            if self.algorithm == "RandomUnderSampler":
                ...
                version = (
                    int(self.algorithm.split("-")[1]) if "-" in self.algorithm else 1
                )
                if version not in [1, 2, 3]:
                    raise ValueError(
                        f"Invalid NearMiss version: {version}. Choose 1, 2, or 3."
                    )
                self._sampler = NearMiss(
                    version=version,
                    sampling_strategy=sampling_strategy_param,
                    random_state=self.random_state,
                )
                ...
                self._sampler = CondensedNearestNeighbour(
                    sampling_strategy=sampling_strategy_param,
                    random_state=self.random_state,
                )
            elif self.algorithm == "EditedNearestNeighbours":
                self._sampler = EditedNearestNeighbours(
                    sampling_strategy=sampling_strategy_param
                )
    ...
    def fit_resample(self, X: np.ndarray, y: np.ndarray):
        ...
        try:
            ...
            print(
                f"Resampled dataset shape using {self.algorithm}: {Counter(y_resampled)}"
            )
            return X_resampled, y_resampled
        except Exception as e:
            print(f"An error occurred during resampling with {self.algorithm}: {e}")
            print(
                "Please check if the algorithm is suitable for your dataset characteristics."
            )
...
if __name__ == "__main__":
    ...
    smote_sampler = ImbalancedSampler(
        algorithm="SMOTE", sampler_type="upsample", target_ratio=0.5, random_state=42
    )
    ...
    print(
        f"Resampled dataset shape (RandomOverSampler, target ratio 1.0): {Counter(y_res_ros)}"
    )
    ...
    adasyn_sampler = ImbalancedSampler(
        algorithm="ADASYN", sampler_type="upsample", target_ratio=0.75, random_state=42
    )
    X_res_adasyn, y_res_adasyn = adasyn_sampler.fit_resample(X_train, y_train)
    print(
        f"Resampled dataset shape (ADASYN, target ratio 0.75): {Counter(y_res_adasyn)}"
    )
    ...
    rus_sampler = ImbalancedSampler(
        algorithm="RandomUnderSampler",
        sampler_type="downsample",
        target_ratio=1.0,
        random_state=42,
    )
    X_res_rus, y_res_rus = rus_sampler.fit_resample(X_train, y_train)
    print(
        f"Resampled dataset shape (RandomUnderSampler, target ratio 1.0): {Counter(y_res_rus)}"
    )
    ...
    nm1_sampler = ImbalancedSampler(
        algorithm="NearMiss-1",
        sampler_type="downsample",
        target_ratio=0.8,
        random_state=42,
    )
    ...
    print(
        f"Resampled dataset shape (NearMiss-1, target ratio 0.8): {Counter(y_res_nm1)}"
    )
    ...
    enn_sampler = ImbalancedSampler(
        algorithm="EditedNearestNeighbours",
        sampler_type="downsample",
        target_ratio=1.0,
        random_state=42,
    )
    ...
    try:
        invalid_sampler = ImbalancedSampler(
            algorithm="UnsupportedAlgorithm", sampler_type="upsample", target_ratio=1.0
        )
    ...
    try:
        invalid_type_sampler = ImbalancedSampler(
            algorithm="SMOTE", sampler_type="invalid", target_ratio=1.0
        )
