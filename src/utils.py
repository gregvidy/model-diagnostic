...
def categorize_terminal_owner(owner):
    ...
    if any(keyword in owner for keyword in ['alibaba', 'distro', 'hellofresh', 'amazon', 'taobao','amzn','ebay', 'shop', 'store', 'market', 'commerce', 'tokopedia', 'shopee', 'blibli', 'lazada', 'zalora', 'temu', 'aliexpress', 'bukalapak']):
        return 'ecommerce'
    elif any(keyword in owner for keyword in ['oculus', 'netflix', 'spotify', 'youtube', 'digital', 'stream', 'media', 'apple music', 'disney+', 'viu', 'iflix', 'crunchyroll', 'scribd', 'kindle']):
        return 'digital products'
    elif any(keyword in owner for keyword in ['bloomberg', 'nba', 'ufc', 'canva', 'samsung', 'zoom', 'discord', 'bitly', 'wordpress', 'classpass']):
        return 'online subscription'
    elif any(keyword in owner for keyword in ['ticket', 'reverbnation']):
        return 'tickets'
    elif any(keyword in owner for keyword in ['starlink', 'finpay', 'dana', 'telkomsel', 'indosat', 'axiata', 'hutchinson', 'pln', 'xl axiata', 'smartfren', 'sp powerpacsg']):
        return 'utilities'
    elif any(keyword in owner for keyword in ['digimap', 'erafone', 'istore', 'ibox', 'electronic city', 'best denki']):
        return 'retail - electronics'
    elif any(keyword in owner for keyword in ['ikea', 'ace hardware', 'informa', 'depo bangunan']):
        return 'retail - furniture'
    elif any(keyword in owner for keyword in ['mall']):
        return 'retail - others'
    elif any(keyword in owner for keyword in ['udemy', 'coursera', 'edx', 'learn', 'education', 'ruang guru', 'zenius', 'skillshare', 'duolingo', 'khan academy']):
        return 'e-learning'
    elif any(keyword in owner for keyword in ['garuda', 'singaporeair', 'sq', 'airbnb', 'booking', 'travel', 'trip', 'tour', 'agoda', 'expedia', 'tiket.com', 'traveloka', 'trip.com']):
        return 'travel'
    elif any(keyword in owner for keyword in ['careem', 'bolt', 'uber', 'lyft', 'grab', 'ride', 'taxi', 'blue bird', 'xanh', 'gojek', 'maxim', 'indrive', 'beam', 'lime']):
        return 'transportation'
    elif any(keyword in owner for keyword in ['apple', 'microsoft', 'google', 'tech', 'software', 'openai', 'midjourney', 'ai', 'dropbox', 'adobe', 'notion', 'slack', 'github']):
        return 'technology'
    elif any(keyword in owner for keyword in ['domino', 'coffee', 'doordash', 'mcdonalds', 'starbucks', 'restaurant', 'food', 'beverage', 'kfc', 'dominos', 'grabfood', 'gofood', 'foodpanda']):
        return 'food & beverage'
    elif any(keyword in owner for keyword in ['blizzard', 'sony', 'nintendo', 'playstation', 'xbox', 'game', 'gaming', 'steam', 'garena', 'mobile legends', 'pubg', 'roblox', 'genshin', 'riot games', 'activation']):
        return 'gaming'
    elif any(keyword in owner for keyword in ['facebook', 'twitter', 'instagram', 'social', 'media', 'tiktok', 'linktree', 'heylink', 'onlyfans']):
        return 'social media'
    elif any(keyword in owner for keyword in ['paper.id', 'pay', 'doku', 'bank', 'paypal', 'finance', 'financial', 'payoneer', 'stripe', 'wise', 'dana', 'ovo', 'gopay', 'linkaja']):
        return 'finance'
    elif any(keyword in owner for keyword in ['dentist', 'health', 'wellness', 'fitness', 'gym', 'mindvalley', 'celebrityfitness', 'strong marriage now', 'betterhelp']):
        return 'health & wellness'
    elif any(keyword in owner for keyword in ['clinic', 'hospital', 'insurance', 'bpjs', 'prudential', 'allianz', 'axa']):
        return 'medical & insurance'
    elif any(keyword in owner for keyword in ['immigration', 'tax', 'police', 'npwp', 'bpjs', 'lta', 'sp powerpacsg']):
        return 'government & public services'
    elif any(keyword in owner for keyword in ['hotel', 'resort', 'inn', 'motel', 'hostel', 'lodging', 'guesthouse', 'bnb', 'marriott', 'hilton', 'hyatt', 'accor', 'pullman', 'citadines', 'fairfield', 'novotel', 'ritz', 'intercontinental', 'holiday inn', 'reddoorz', 'airy', 'oyo', 'aston', 'santika', 'pop hotel', 'ibis', 'amaris', 'favehotel', 'luxehotel', 'mandarin oriental', 'shangri-la', 'four seasons', 'capella', 'the fullerton', 'raffles hotel', 'kempinski']):
        return 'hotels & resorts'
    elif any(keyword in owner for keyword in ['petrol', 'gas', 'fuel', 'shell', 'pertamina', 'esso', 'caltex', 'total', 'bp', 'petronas', 'mobil', 'gas station', 'spbu', 'bensinstation', 'refuel', 'refueling']):
